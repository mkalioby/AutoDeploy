...
from . import models
import os
from autoDeploy import settings as settings
...
class ProjectsForm(forms.ModelForm):
    ...
    deployment_link = forms.CharField(label='Deployment Link',
                                      widget=forms.TextInput(attrs={'class': 'form-control', 'size': 30}))
    emailUsers = forms.CharField(required=False, label='Users emails',
                                 help_text="comma seprated list of emails of users to notify when new version deployed",
                                 widget=forms.TextInput(attrs={'class': 'form-control', 'size': 30}))
    default_server = forms.ModelChoiceField(queryset=models.Server.objects.all(), empty_label="Select",
                                            widget=forms.Select(attrs={"class": "form-control"}),
                                            label="Default Server")
    sshKey = forms.ModelChoiceField(queryset=models.SSHKey.objects.all(), empty_label="Select", label="SSH Key",
                                    widget=forms.Select(attrs={"class": "form-control"}))
    default_branch = forms.CharField(label='Default Branch',
                                     widget=forms.TextInput(attrs={'class': 'form-control', 'size': 30}))
    ...
    class Meta:
        ...
        fields = (
        "name", "repo", "repo_link", "working_dir", "update_style", "default_branch", "default_server", "repo_type",
        "sshKey", "deployment_link", "cfile", "emailUsers")
